...
class TextureNet(nn.Module):
    def __init__(self, cat_num):
        ...
        self.mnet.classifier[0] = nn.Linear(1280, 256)
        self.mnet.classifier[1] = nn.Linear(256, cat_num)
...
def load_optimized_qtables(args):
    with open(args.qtable, 'r') as fp:
        info = json.loads(fp.read())
        qs = np.array([int(q) for q in info.keys()])
        best_q = qs[np.argmin(np.abs(qs - args.quality))]
        print(f'load {args.qtable} at quality {best_q}')
        qtables = info[str(best_q)]
    return qtables
...
def exec_encode(args):
    ...
    model = TextureNet(TEXTURE_CATS).to(device)
    model.load_state_dict(torch.load(args.model, map_location=device))

    qtables = load_optimized_qtables(args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--qtable', default='./data/qtables.json', type=str, help='optimized qtables')
